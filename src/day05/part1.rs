...
impl<'o> OrderingMask<'o> {
    ...
    fn push(&mut self, num: u8) -> bool {
        self.ord_map[num]
            .iter()
            .for_each(|&idx| self.allowed[idx as usize] = false);

        self.allowed[num as usize]
    }
}
...
pub fn process(input: &str) -> Result<String, Error> {
    ...
    for line in update_def.lines() {
        ...
        if nums.iter().all(|&num| mask.push(num)) {
            sum += nums[nums.len() / 2] as u32;
        }
    }
